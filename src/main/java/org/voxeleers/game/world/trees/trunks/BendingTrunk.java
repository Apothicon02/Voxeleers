package org.voxeleers.game.world.trees.trunks;
    ...
    public static Pair<Map<Vector3i, Vector2i>, Set<Vector3i>> generateTrunk(Random random, int oX, int oY, int oZ, boolean crown, int count, int trunkHeight, int blockType, int blockSubType) {
        ...
        for (int trunks = 0; trunks <= extra+1; trunks++) {
            ...
            for (int height = origin.y()-1; height <= maxHeight; height++) {
                ...
                if (height == maxHeight) {
                    canopies.add(VoxeleersMath.addVec(pos, 0, 1, 0));
                    if (crown && height > 12) {
                        addToMap(map, VoxeleersMath.addVec(pos, 2, 0, 0), new Vector2i(3, 0));
                        addToMap(map, VoxeleersMath.addVec(pos, 1, -1, 0), new Vector2i(3, 0));
                        addToMap(map, VoxeleersMath.addVec(pos, 0, 0, 2), new Vector2i(3, 0));
                        addToMap(map, VoxeleersMath.addVec(pos, 0, -1, 2), new Vector2i(3, 0));
                        addToMap(map, VoxeleersMath.addVec(pos, -2, 0, 0), new Vector2i(3, 0));
                        addToMap(map, VoxeleersMath.addVec(pos, -1, -1, 0), new Vector2i(3, 0));
                        addToMap(map, VoxeleersMath.addVec(pos, 0, 0, -2), new Vector2i(3, 0));
                        addToMap(map, VoxeleersMath.addVec(pos, 0, -1, -2), new Vector2i(3, 0));
